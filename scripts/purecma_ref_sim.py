from cma import purecma_reference, ff
...
for i in range(1, 29): # for every CEC test function 1-28 (i)
    for j in range(2, 7):
        for k in range(51):
            ...
            xs[k] = x.tolist()
            purecma_reference.fmin(no = k, xstart = xs[k], sigma = 20/3, ps = 4*dim, n = j, mi = 2*dim, fn = i)
'''
'''xs=[]
dim = 2
...
'''
dim = 2
res = purecma_reference.fmin(1, dim * [0.5], sigma = 20/3, ps = 4*dim, n = 4, mi = 2*dim, fn = 1, verb_disp=100)
